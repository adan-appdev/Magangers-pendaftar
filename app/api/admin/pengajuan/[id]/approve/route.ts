...
export async function POST(req: Request, { params }: { params: any }) {
  ...
  if (found.error) {
    return NextResponse.json({ message: "Query pengajuan error", detail: found.error.message }, { status: 500 });
  }
  if (!found.pengajuan) {
    return NextResponse.json({ message: "Pengajuan tidak ditemukan", detail: `id=${id}` }, { status: 404 });
  }
  ...
  if (upPengajuanErr) {
    return NextResponse.json({ message: "Update pengajuan error", detail: upPengajuanErr.message }, { status: 500 });
  }

  // ✅ status peserta: verifikasi (menunggu dijadwalkan wawancara)
  ...
  if (upPesertaErr) {
    return NextResponse.json({ message: "Update peserta error", detail: upPesertaErr.message }, { status: 500 });
  }

  return NextResponse.json({ message: "OK", detail: `matched_by=${found.used}, pengajuan_id=${pengajuanId}` });
}